...
async def cmd_donate(message: Message, command: CommandObject):
    ...
    if command.args is None or not command.args.isdigit() or not 1 <= int(command.args) <= 2500:
        await message.answer(
            text=_('Пожалуйста, введите сумму в формате <code>/donate [ЧИСЛО]</code>\n'
    ...
    await message.answer_invoice(
        title=_("Донат"),
        description=_("На сумму в {amount} ⭐️").format(amount=amount),
        prices=prices,
        provider_token="",
...
async def cmd_paysupport(message: Message, state: FSMContext):
    ...
    t_id = data.get('t_id', _('ID транзакции не найден'))
    await message.answer(
...
async def cmd_terms(message: Message):
    await message.answer(_("<b>Условия использования бота</b>\n\n"
...
async def cmd_refund(message: Message, bot: Bot, command: CommandObject):
    ...
    if t_id is None:
        await message.answer(_("Пожалуйста, укажите идентификатор транзакции в формате <code>/refund [id]</code>, "
                                "где [id] это идентификатор транзакции, который вы получили после доната.\n\n"
